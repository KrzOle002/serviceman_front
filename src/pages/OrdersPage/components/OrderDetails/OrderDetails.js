...
const OrderDetails = ({ handleChangeOpen, activeOrder, setOrders, orders }) => {
	...
	const removeOrder = async e => {
		e.preventDefault()
		await axios
			.delete(`http://192.168.206.71:5000/api/order/${activeOrder}`)
			.then(res => {
	...
	const updateOrder = async data => {
		await axios
			.post(`http://192.168.206.71:5000/api/order/${activeOrder}`, data)
			.then(res => {
				setSaving(true)
				handleChangeOpen()
	...
	const finishOrder = async e => {
		e.preventDefault()
		await axios
			.post(`http://192.168.206.71:5000/api/order/finish/${activeOrder}`)
			.then(res => {
				setClosing(true)
				handleChangeOpen()
	...
	const resumeOrder = async e => {
		e.preventDefault()
		await axios
			.post(`http://192.168.206.71:5000/api/order/resume/${activeOrder}`)
			.then(res => {
				setClosing(true)
				handleChangeOpen()
	...
	useEffect(() => {
		const getOrder = async () => {
			await axios
				.get(`http://192.168.206.71:5000/api/order/${activeOrder}`)
				.then(res => {
					setOrder(res.data)
					setLoading(false)
				})
